...
field_data <- read_csv("Field_data_BIO102.csv", na = c(""), col_types = "cnctDcn") %>% 
  mutate(
    Start = as.POSIXct(paste(Date, Starting_time), format="%Y-%m-%d %H:%M:%S") - startcrop, #converting the date as posixct, pasting date and starting time together and cropping the start
...
co2conc_bio102 <- fuzzy_left_join( #there is probably a better way to do that than fuzzy join, I need to find out. Fuzzy join is using too much memory
  fluxes, field_data,
  by = c(
    "Datetime" = "Start",
    "Datetime" = "End"
